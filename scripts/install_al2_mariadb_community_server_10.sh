...
mariadb_server_root_password="${mariadb_server_root_password:-Welcome1!}"   # [optional] root password (defaults to 'Password1234!').
...
sudo mkdir -p ${devops_home}/installs/mariadb
sudo cd ${devops_home}/installs/mariadb

# configure mariadb yum repository. ----------------------------------------------------------------
#sudo rm -f mariadb_repo_setup
#sudo wget --no-verbose https://downloads.mariadb.com/MariaDB/mariadb_repo_setup
#sudo chmod 755 ./mariadb_repo_setup
...
#mariadb_secure_install_cmd=$(printf "mysql_secure_installation <<< \$\'\\\\n%s\\\\n%s\\\\n%s\\\\n%s\\\\n%s\\\\n%s\\\\n%s\\\\n%s\\\\n\'\n" n Y ${mariadb_server_root_password} ${mariadb_server_root_password} Y n Y Y)
