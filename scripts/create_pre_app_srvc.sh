...
service_name="appdpreapp.service"
service_filepath="${systemd_dir}/${service_name}"
service_user="ec2-user"
service_group="ec2-user"
service_scripts_home="/home/ec2-user/environment/migration_workshop/applications/pre-migration"
...
if [ -d "$systemd_dir" ]; then
  ...
  echo "ExecStart=${service_scripts_home}/start_app.sh" >> "${service_filepath}"
  echo "ExecStop=${service_scripts_home}/stop_app.sh" >> "${service_filepath}"
  echo "" >> "${service_filepath}"
